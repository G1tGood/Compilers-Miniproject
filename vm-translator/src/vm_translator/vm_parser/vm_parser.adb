...
package body VM_Parser is
   ...
   begin
      ...
      if To_String (Command.First_Element) in "add" | "sub" | "neg" | "eq" | "gt" | "lt" | "and" | "or" | "not" then
         ...
         FirstArg := Command.Element (Index => 2);
         SecondArg := Natural'Value (To_String (Command.Element (Index => 3)));
      elsif Command.First_Element = "label" then
         CurrentCommandType := C_LABEL;
         FirstArg := Command.Element (Index => 2);
      elsif Command.First_Element = "goto" then
         CurrentCommandType := C_GOTO;
         FirstArg := Command.Element (Index => 2);
      elsif Command.First_Element = "if-goto" then
         CurrentCommandType := C_IF;
         FirstArg := Command.Element (Index => 2);
      elsif Command.First_Element = "return" then
         CurrentCommandType := C_RETURN;
      elsif Command.First_Element = "call" then
         CurrentCommandType := C_CALL;
         FirstArg := Command.Element (Index => 2);
         SecondArg := Natural'Value (To_String (Command.Element (Index => 3)));
      elsif Command.First_Element = "function" then
